...
function dazel_register_components( $components ) {
...
Hooks::prefixed_add_filter( 'components', 'dazel_register_components', 20 );
Theme::load(
	array(
 ...
 * @return Theme
 */
function dazel_theme() {
	return Theme::getInstance();
}

function dazel_assets() {
	return dazel_theme()->getAssetsManager();
}
...
add_action('after_setup_theme', function() {
    dazel_theme()->register_menus(
        array(
            'header-menu' => esc_html__( 'Header Menu', 'dazel' ),
            'footer-menu' => esc_html__( 'Footer Menu', 'dazel' ),
        )
    );
}, 1);
if ( ! apply_filters( 'kubio_is_enabled', false ) ) {
	dazel_assets()
		...
		->registerStylesheet( 'dazel-theme', '/theme/theme.css' );
	dazel_assets()->loadLocalGoogleFonts();
	//	foreach ( Defaults::get( 'fonts', array() ) as $family => $variants ) {
	//		dazel_assets()->addGoogleFont( $family, $variants );
	//	}

	add_action( 'wp_enqueue_scripts', 'dazel_print_color_scheme', 0 );
} else {
	dazel_assets()
		->registerStyle( 'dazel-theme', dazel_assets()->getBaseURL() . '/theme/fse-base-style.css' );
	Hooks::prefixed_add_filter( 'skip_google_fonts', '__return_true' );
}

function dazel_theme_add_woocommerce_support() {
	add_theme_support(
		'woocommerce',
		array(
			'product_grid' => array(
...
add_action( 'after_setup_theme', 'dazel_theme_add_woocommerce_support' );

add_filter( 'kubio/activation/after_activation_redirect_url', 'dazel_after_activation_redirect_url' );
function dazel_after_activation_redirect_url( $url ) {
...
function dazel_get_builder_plugin_slug() {
...
function dazel_theme_plugins( $plugins ) {

	$plugin_slug = dazel_get_builder_plugin_slug();

	$plugins[ $plugin_slug ] = array(
		'name'        => $plugin_slug === 'kubio-pro' ? __( 'Kubio PRO', 'dazel' ) : __( 'Kubio', 'dazel' ),
...
Hooks::prefixed_add_filter( 'theme_plugins', 'dazel_theme_plugins' );
...
Hooks::prefixed_add_action(
	...
	function ( $slug ) {
		...
		if ( $slug === dazel_get_builder_plugin_slug() ) {
                ...
                <div class="kubio-admin-row get-started-imported notice notice-success">
                    <div>
                        <p class="imported-title">
							...
							echo esc_html(
								sprintf(
									__( '%s design has been successfully imported!', 'dazel' ),
									dazel_theme()->getName()
								)
							);
							?>
                        ...
                        <p class="imported-subtitle">
							...
							echo esc_html(
								sprintf(
									__( '%s design has been successfully imported! You can take a look at your new design or start editing it', 'dazel' ),
									dazel_theme()->getName()
								)
							);
							?>
                    ...
                    <div class="button imported-view-site-button">
                        <a href="<?php echo esc_url( site_url() ); ?>">
							<?php echo esc_html( __( 'View site', 'dazel' ) ); ?>
                        </a>
...
function dazel_try_online_url() {
	return 'https://kubiobuilder.com/go/try-theme/' . get_template();
}

add_filter( 'kubio/editor-try-online/url', 'dazel_try_online_url', 10 );

function dazel_render_header_style() {

	?>
    <base target="_top">
	<?php

	dazel_theme()->get( 'css' )->render();
}
add_action( 'wp_head', 'dazel_render_header_style', 100 );
