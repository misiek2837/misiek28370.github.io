...
/*!**************************************!*\
  ...
  \**************************************/
(function ($) {
  var $notice_container = $(".dazel-admin-big-notice--container");
  var selectedFrontPage = 0;

  if (!window.dazel_admin) {
    return;
  }

  var _window$dazel_admin = window.dazel_admin,
      builderStatusData = _window$dazel_admin.builderStatusData,
      getStartedData = _window$dazel_admin.getStartedData;
  ...
  function toggleProcessing(value) {
    $(window).on("beforeunload.dazel-admin-big-notice", function () {
      return true;
    });

    if (value) {
      $(".dazel-admin-big-notice").addClass("processing");
      $(".dazel-admin-big-notice .action-buttons").fadeOut();
    } else {
      $(".dazel-admin-big-notice").removeClass("processing");
    }
  }
  ...
  function installBuilder(callback) {
    ...
    $.get(builderStatusData.install_url).done(function () {
      ...
      $(window).off("beforeunload.dazel-admin-big-notice");
    });
  }
  ...
  $notice_root = $notice_container.closest(".dazel-admin-big-notice");
  $custom_close_button = $notice_root.find(".dazel-notice-dont-show-container");

  if ($custom_close_button.length) {
    $custom_close_button.on("click", function () {
      disableNotice();
      $notice_container.closest(".dazel-admin-big-notice").find("button.notice-dismiss").click();
    });
  } else {
  ...
  window.dazel_admin.showOverlay = showOverlay;
})(jQuery);
/******/ })()
;
